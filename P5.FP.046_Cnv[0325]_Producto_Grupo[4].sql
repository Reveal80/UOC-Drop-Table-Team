...
BEGIN
    ...
    INSERT INTO shows_semanales (año, mes, semana, catname, eventname, localidad, starttime)
    SELECT 
        date.year AS año,
        date.month AS mes,
        date.week AS semana,
...
-- Lo usaremos en la proxima pregunta, pero si lo ejecutamos manualmente podemos comprobar que afectan a 119 eventos.
...
-- Vemos que el Value es: /var/lib/mysql-files/ , así que usaremos esta ruta donde exportar el TXT.

SELECT * FROM shows_semanales
INTO OUTFILE '/var/lib/mysql-files/shows_semanales.txt' -- Configurar el parametro anterior donde exportaremos el TXT. en Windows C:\...
FIELDS TERMINATED BY ',' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n';

-- Si todo funciona bien deberia aparecer un mensaje como este: 119 row(s) affected
-- Y en la consola de linux si hacemos un ls -rw-r----- 1 mysql mysql 9714 Apr 26 11:25 shows_semanales.txt
...
-- CALL profit_sellers();
-- Y si funciona bien ver algo asi: 13:26:56	SELECT * FROM TICKIT.ventas_entradas LIMIT 0, 1000	16 row(s) returned	0.000 sec / 0.000 sec
...
BEGIN
    -- Eliminar registros de ventas de más de 5 años de antigüedad. Hacienda nos dice que solo se deben guardar los ultimos 5 años.
    DELETE FROM sales
    WHERE listid IN (
