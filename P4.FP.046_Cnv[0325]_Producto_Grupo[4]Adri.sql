...
-- Índices en sales
--Se utiliza para consultas como LEFT JOIN sales ON users.userid = sales.buyerid, seguidas de WHERE sales.buyerid IS NULL. Si no hay un índice en buyerid, MySQL realiza un escaneo completo sobre sales en cada fila de users. Este índice permite buscar rápidamente las coincidencias por comprador, mejorando el rendimiento del JOIN y del filtro.
CREATE INDEX idx_sales_buyerid ON sales(buyerid);
--Similar al anterior, este índice optimiza la consulta donde se buscan usuarios que no han vendido (users.userid = sales.sellerid). Facilita búsquedas por vendedores en las combinaciones LEFT JOIN, reduciendo los tiempos de ejecución.
CREATE INDEX idx_sales_sellerid ON sales(sellerid);

-- Índice en users
--userid es clave primaria, por lo tanto ya suele estar indexado automáticamente, pero se declara explícitamente para indicar que su uso es clave en los JOINs con sales y listing. Mejora el rendimiento al combinar tablas desde la perspectiva de users.
...
--Esta combinación de columnas se usa en la cláusula WHERE d.month = @mes AND d.year = 2008. Tener un índice conjunto permite que la consulta filtre directamente usando ese índice, en lugar de recorrer la tabla completa de fechas.
...
-- Permisos: SELECT, INSERT, UPDATE sobre sales. Y SELECT sobre users y listing.
	...
	GRANT SELECT, INSERT, UPDATE ON sales TO rol_gestor_ventas;
	GRANT SELECT ON users, listing TO rol_gestor_ventas;
...
-- Permisos: SELECT sobre event, date, sales, users.
	...
	GRANT SELECT ON event, date, sales, users TO rol_analista_eventos;

-- Rol 4. rol_admin_db
-- Permisos: Todos los privilegios necesarios para administración: ALL PRIVILEGES.
	CREATE ROLE rol_admin_db;
	GRANT ALL PRIVILEGES ON *.* TO rol_admin_db;